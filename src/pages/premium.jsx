    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-black px-6 py-16 font-['Inter']">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl w-full">
                {/* insteal of writing carfds two time we use map  */}
                {plans.map((plan, i) => (
                    <div
                        key={i}
                        className="relative flex flex-col justify-between bg-[#111] border border-neutral-800 rounded-lg p-8 hover:border-purple-500 transition"
                    >
                        {/* Badge */}
                        {plan.badge && (
                            <div className="absolute -top-3 left-6 bg-purple-600 text-xs font-semibold px-3 py-1 rounded-full text-white">
                                {plan.badge}
                            </div>
                        )}
